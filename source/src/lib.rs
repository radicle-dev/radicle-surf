...
pub enum Error {
    /// We expect at least one [`crate::source::Revisions`] when looking at a
...
/// Will return [`Error`] if the project doesn't exist or the surf interaction
/// fails.
pub fn commit_header(browser: &mut Browser<'_>, sha1: git2::Oid) -> Result<CommitHeader, Error> {
    browser.commit(sha1)?;
...
pub fn commit(browser: &mut Browser<'_>, sha1: git2::Oid) -> Result<Commit, Error> {
    ...
    // Known commits always have at least one branch. If this isn't the case, it's a
    // bug.
    let branch = Branch(
        branch
...
///
/// Since a replicated peer is one that we have locally, this means that they
/// have a user profile to build the revision with.
///
/// However, if the [`Peer`] does not have any related branches this function
/// will return `None`.
///
/// # Errors
///
...
{
    match peer {
        Category::Local { identifier, user } => local_revisions(browser, identifier, user),
        Category::Remote { identifier, user } => remote_revision(browser, identifier, user),
    }
}
